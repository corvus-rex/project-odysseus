...
export default {
    ...
    methods: {
      ...
      getDraft() {
        ...
        axios.post(serverSide.getDraft, {draftID: this.draftID})
        .then((res) => {
          if(this.publisher._id != res.data.draft[0].publisher) {
            this.$router.push({name: "newsroom"})
          }
          else {
            this.newsTitle = res.data.draft[0].title
            this.newsDescription = res.data.draft[0].description
            this.newsTopic = res.data.draft[0].topic
            this.newsTags = res.data.draft[0].tags
            this.newsLocations = res.data.draft[0].locations
            this.news = res.data.draft[0].article
          }
        })
      },
      editDraft() {
